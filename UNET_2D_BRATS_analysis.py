...
def find_best_conf():
    ...
    for conf in np.arange(0, 1.018, 0.01):
        conf = float(conf)
        DSC_scores = []
        ...
        DSC_avg.append(np.mean(DSC_scores))
        confidences.append(conf)

    plt.plot(confidences, DSC_avg)
    max_DSC = max(DSC_avg)
    best_conf = confidences[DSC_avg.index(max_DSC)]
    return max_DSC, best_conf
